package cn.javayuli.cloud.common.sentinel.feign;
 ...
 * @date 2020/6/9
 */
public class YuLiSentinelInvocationHandler implements InvocationHandler {

	private static final Logger LOGGER = LoggerFactory.getLogger(YuLiSentinelInvocationHandler.class);
	...
	YuLiSentinelInvocationHandler(Target<?> target, Map<Method, InvocationHandlerFactory.MethodHandler> dispatch,
			...
			if (methodMetadata == null) {
				...
				try {
					...
					if (fallbackFactory != null) {
						...
						// 若是R类型 执行自动降级返回R
						if (Rest.class == method.getReturnType()) {
                            LOGGER.error("feign 服务间调用异常", ex);
							return Rest.fail(ex.getLocalizedMessage());
						}
						else {
							throw ex;
						}
	...
	public boolean equals(Object obj) {
		if (obj instanceof com.alibaba.cloud.sentinel.feign.SentinelInvocationHandler) {
			YuLiSentinelInvocationHandler other = (YuLiSentinelInvocationHandler) obj;
