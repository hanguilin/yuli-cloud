...
public class YuLiUserAuthenticationConverter implements UserAuthenticationConverter {
	...
	public Map<String, ?> convertUserAuthentication(Authentication authentication) {
		...
		response.put(SecurityConstant.DETAILS_USER_ACCOUNT, authentication.getName());
	...
	public Authentication extractAuthentication(Map<String, ?> map) {
		...
		if (map.containsKey(SecurityConstant.DETAILS_USER_ACCOUNT)) {
			// 获取用户权限
			Collection<? extends GrantedAuthority> authorities = getAuthorities(map);

			String id = (String) map.get(SecurityConstant.DETAILS_USER_ID);
			String account = (String) map.get(SecurityConstant.DETAILS_USER_ACCOUNT);
			YuLiUser user = new YuLiUser(id, account, N_A, authorities);
