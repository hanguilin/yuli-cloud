...
import cn.javayuli.cloud.system.api.mapper.SysUserMapper;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Rest<Boolean> deleteUser(String ids) {
        ...
        if (CollUtil.isNotEmpty(idList)) {
            ...
            if (cache != null) {
                idList.forEach(id -> cache.evictIfPresent(id));
            }
        }
        return Rest.success();
    }
     ...
     * @return
     */
    @CacheEvict(value = CacheNames.USER_DETAILS, key = "#user.id", condition = "#user.id!=null")
    @Transactional(rollbackFor = Exception.class)
    @Override
